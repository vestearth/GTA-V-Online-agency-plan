...
| All Cars Needed | latest `data/weekly_planning_*.json` + `data/references/vehicle_prices.yaml` | Generated in Phase 1 from unique weekly vehicle appearances with known base prices and a buy path. Prize Ride and Lucky Wheel reward vehicles stay visible in the spotlight, but their prices are excluded from this total; unresolved prices are skipped and reported. |
